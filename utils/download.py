...
async def download_file(
    ...
    for attempt in range(max_retries):
        try:
            status = await _attempt_download(
                session, url, file_path, timeout_total, timeout_connect, chunk_size
            )
            if status == 200:
                return True
            else:
...
async def download_files(
    ...
    if failed_count > 0:
        summary_msg = (
            f"⚠ Download completed: {success_count}/{len(urls)} "
...
def _log_timeout_error(filename: str, attempt: int, max_retries: int) -> None:
    ...
    if attempt == max_retries - 1:
        timeout_msg = (
            f"\n✗ Timeout downloading '{filename}' after {max_retries} attempts\n"
        )
        sys.stderr.write(timeout_msg)


def _log_general_error(
    filename: str, error: Exception, attempt: int, max_retries: int
) -> None:
...
def _log_http_error(filename: str, status: int, attempt: int, max_retries: int) -> None:
    ...
    if attempt == max_retries - 1:
        error_msg = f"\n✗ HTTP {status} for '{filename}' after {max_retries} attempts\n"
        sys.stderr.write(error_msg)
